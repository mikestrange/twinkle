package org.web.sdk.display.engine 
{
	...
	final public class Phoebus 
	{
		...
		public function Phoebus() { throw Error('no do'); }
		...
		public static function stop():Boolean
		{
			if (_isopen && _isplay)
			{
				...
				for each(step in stepVector) {
					_root.removeEventListener(Phoebus.ENTER_FRAME, step.step);
				}
				return true;
			}
			return false;
		...
		public static function play():Boolean
		{
			if (_isopen && !_isplay) 
			{
				...
				var step:IStepper;
				for each(step in stepVector) {
					_root.addEventListener(Phoebus.ENTER_FRAME, step.step);
				}
				return true;
			}
			return false;
		...
		public static function run(step:IStepper):Boolean
		{
			if (_isopen && !isRegister(step)) 
			{
				...
				if (_isplay) _root.addEventListener(Phoebus.ENTER_FRAME, step.step);
				return true;
			}
			return false;
		}
